...
def write(data_frame: DataFrame, connection: Connection, name: str, if_exists: Literal["fail", "replace", "append", "delete_rows"] = "append", index: bool = False, dtype: DtypeArg | None = None, verbose: bool = False) -> None:
    data_frame.to_sql(name, connection, if_exists=if_exists, index=index, dtype=dtype)
